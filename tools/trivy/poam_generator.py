"""
Module for generating POAMs from findings.
"""
...
def parse_trivy_id(poam_id: str) -> Tuple[int, int]:
    """
    Parse a Trivy POAM ID into year and sequence components.
    
    Args:
        poam_id: POAM ID in format YYYY-TRIVYXXXX
    ...
    """
    match = re.match(r'^(\d{4})-TRIVY(\d{4,})$', poam_id)
    if not match:
        raise ValueError(f"Invalid Trivy POAM ID format: {poam_id}")
    
    year = int(match.group(1))
    sequence = int(match.group(2))
    return year, sequence

def get_next_trivy_id(existing_poam_ids: List[str], current_year: int = None) -> str:
    """
    Generate the next available Trivy POAM ID.
    ...
    Returns:
        Next available POAM ID in YYYY-TRIVYXXXX format
    """
    ...
    for poam_id in existing_poam_ids:
        try:
            year, sequence = parse_trivy_id(poam_id)
            if year == current_year and sequence > max_sequence:
                max_sequence = sequence
        except ValueError:
            continue  # Skip invalid IDs
    
    return f"{current_year}-TRIVY{max_sequence + 1:04d}"
...
def generate_poams_from_findings(findings: List[Finding], existing_poam_ids: List[str], current_year: int = None) -> List[Tuple[List[Finding], PoamEntry]]:
    ...
    next_id = get_next_trivy_id(existing_poam_ids, current_year)
    ...
    for findings_list in sorted_groups:
        ...
        next_id = get_next_trivy_id([*existing_poam_ids, next_id], current_year)
